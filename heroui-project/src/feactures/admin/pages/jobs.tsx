...
import { Card, CardBody, CardHeader, CardFooter, Button, Input, Chip, Divider, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Textarea, Select, SelectItem } from "@heroui/react";
...
const jobsMock: Job[] = [
  ...
  },
  {
    ...
    description: "Estamos en búsqueda de un Analista de Marketing Digital para gestionar campañas publicitarias y analizar métricas de rendimiento.",
    requirements: [
  ...
  },
  {
    ...
    description: "Buscamos un Especialista en Recursos Humanos para gestionar procesos de reclutamiento, selección y desarrollo de personal.",
    requirements: [
...
export const AdminJobs: React.FC = () => {
  ...
  const [statusFilter, setStatusFilter] = React.useState<"all" | "active" | "closed" | "draft">("all");
  ...
  const [applicants, setApplicants] = React.useState<Applicant[]>(applicantsMock);
  ...
  const validateForm = () => {
    ...
    if (!formData.closingDate) {
      ...
      if (selectedDate < today) {
        newErrors.closingDate = "La fecha de cierre debe ser igual o posterior a hoy";
        isValid = false;
      }
    }
  ...
  // Change job status
  const handleChangeStatus = (jobId: string, newStatus: "active" | "closed" | "draft") => {
    const updatedJobs = jobs.map(job => 
      job.id === jobId 
    ...
    addToast({
      ...
      description: `La convocatoria ahora está ${newStatus === "active" ? "activa" : newStatus === "closed" ? "cerrada" : "en borrador"}`,
      color: "success"
    });
  };
  ...
  return (
    ...
    >
                ...
                >
                  <div className="flex-grow">
                    ...
                    <p className="text-small text-default-500">
                      Creada: {formatDate(job.createdAt)} | Cierra: {formatDate(job.closingDate)} | 
                ...
                <div>
                  ...
                  <ul className="list-disc pl-5 space-y-1">
                    {selectedJob.requirements.map((req, index) => (
                      <li key={index} className="text-default-700">{req}</li>
                    ))}
                  </ul>
                </div>
              </ModalBody>
      ...
      {/* Applicants modal */}
      <Modal isOpen={isApplicantsModalOpen} onOpenChange={setIsApplicantsModalOpen} size="3xl">
        <ModalContent>
          {(onClose) => selectedJob && (
            <>
              <ModalHeader className="flex flex-col gap-1">
              ...
              <ModalBody>
                <div className="flex justify-between items-center mb-4">
                  <p className="text-default-700">
                    <span className="font-medium">{selectedJob.applicants}</span> postulantes
                  </p>
                </div>
                ...
                {applicants.length > 0 ? (
                  <div className="space-y-4">
                    {applicants.map((applicant) => (
                      <div key={applicant.id} className="p-4 bg-content2 rounded-lg flex flex-col sm:flex-row justify-between gap-3">
                        <div>
                          ...
                          <p className="text-small text-default-500">{applicant.email} | {applicant.phone}</p>
                          <p className="text-tiny text-default-400">Postulación: {new Date(applicant.applicationDate).toLocaleDateString()}</p>
                        </div>
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 self-end sm:self-center">
                          ...
                          >
                            ...
                            <SelectItem key="interviewed">Entrevistado</SelectItem>
                            <SelectItem key="selected">Seleccionado</SelectItem>
                          ...
                          <Button
                            ...
                            color="primary"
                            variant="flat"
                            startContent={<Icon icon="lucide:file-text" width={16} height={16} />}
                          >
                            Ver CV
                          </Button>
                        </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-medium text-foreground-800">No hay postulantes</h3>
                    <p className="text-default-500 mt-2">Esta convocatoria aún no tiene postulantes.</p>
                  </div>
                )}
              </ModalBody>
              <ModalFooter>
